<div>
            ...
            <div class="inventory-header w-100 mb-3">
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center w-100 gap-3">
                    ...
                    <div class="d-flex gap-2 justify-content-md-end">
                        <button class="btn btn-outline-success" wire:click="exportProducts">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end">
                                                        ...
                                                        <li>
                                                            <a class="dropdown-item"
                                                                href="{{ route('admin.product-history', $product->id) }}"
                                                                target="_blank">
                                ...
                                <div class="p-4 text-center">
                                    <div class="product-image-container mb-4 position-relative">
                                        ...
                                        <img src="{{ $viewProductImageUrl }}"
                                            ...
                                            data-fallback="{{ asset('images/product.jpg') }}"
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="info-section mb-4">
                                        <div class="section-header d-flex align-items-center mb-3">
                                            ...
                                            <h6 class="fw-bold mb-0 text-dark">Current Batch (FIFO)</h6>
                                        </div>
                                        <div class="alert alert-info border-0 shadow-sm mb-3">
                                            <div class="d-flex align-items-center mb-2">
                                                <i class="bi bi-info-circle me-2"></i>
                                                <strong>Active Batch: {{ $viewProduct->currentBatchInfo['batch_number'] }}</strong>
                                            </div>
                                            <div class="row g-2 mt-2">
                                                <div class="col-md-4">
                                                    <small class="text-muted d-block">Remaining Quantity</small>
                                                ...
                                                <div class="col-md-4">
                                                    <small class="text-muted d-block">Batch Selling Price</small>
                                                    <strong class="text-primary">Rs. {{ number_format($viewProduct->currentBatchInfo['selling_price'], 2) }}</strong>
                                                </div>
                                            </div>
                                        </div>

                                        @if($viewProduct->allBatches && count($viewProduct->allBatches) > 1)
                                        <div class="mt-3">
                                            <h6 class="fw-semibold mb-2 text-muted">All Active Batches:</h6>
                                            <div class="table-responsive">
                                                <table class="table table-sm table-bordered">
                                                    ...
                                                    <tbody>
                                                        @foreach($viewProduct->allBatches as $batch)
                                                        <tr class="{{ $loop->first ? 'table-success' : '' }}">
                                                            ...
                                                            <td>Rs. {{ number_format($batch['supplier_price'], 2) }}</td>
                                                            <td>Rs. {{ number_format($batch['selling_price'], 2) }}</td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Product selection (checkbox) handlers
            document.addEventListener('change', function(e) {
                // Individual product checkbox -> toggle class on the row
                if (e.target && e.target.classList && e.target.classList.contains('product-select-checkbox')) {
                    const checkbox = e.target;
                    const tr = checkbox.closest('tr');
                    if (!tr) return;
                    if (checkbox.checked) tr.classList.add('product-selected');
                    else tr.classList.remove('product-selected');
                }

                // Select all checkbox -> toggle class on all rows
                if (e.target && e.target.id === 'select-all-products') {
                    ...
                    checkboxes.forEach(cb => {
                        ...
                        if (!tr) return;
                        if (selectAll.checked) tr.classList.add('product-selected');
                        else tr.classList.remove('product-selected');
                    });
                }
            });
