...
class Analytics extends Component
{
    ...
    {
        // Get POS sales statistics only (user only uses POS)
        $salesStats = Sale::where('sale_type', 'pos')
            ->select(
        ...
        // totalRevenue = gross POS sales (invoice total, not just collected)
        $this->totalSales = $salesStats->total_sales ?? 0;
        $this->totalDueAmount = $salesStats->total_due ?? 0;
        $this->totalRevenue = $this->totalSales;
        ...
        // Get previous month's POS revenue for comparison
        $previousMonthSales = Sale::where('sale_type', 'pos')
        ...
        // Get fully paid POS invoices data
        $fullPaidData = Sale::where('sale_type', 'pos')
        ...
        // Get partially paid POS invoices data
        $partialPaidData = Sale::where('sale_type', 'pos')
    ...
    {
        // Get monthly POS sales data for the last 12 months
        ...
        // Get POS invoice status distribution
    ...
    {
        return view('livewire.invontery.admin.analytics', [
