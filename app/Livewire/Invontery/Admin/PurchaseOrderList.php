...
class PurchaseOrderList extends Component
{
    ...
    public function selectProduct($id)
    {
        ...
        // BUG-3 FIX: Warn user if product has no price on record
        if (!$price || floatval($price) <= 0) {
            $price = 0;
            $this->js("Swal.fire({
                icon: 'warning',
                title: 'No Price Found',
                text: 'Product \"{$product->name}\" has no supplier price on record. Please enter the price manually before saving.',
    ...
    public function getViewOrderTotalProperty()
    {
        ...
        return floatval(collect($this->selectedOrder->items)->sum(function ($item) {
    ...
    <body>
        ...
        $totalAmount = floatval(collect($order->items)->sum(function ($item) {
            $qty = $item->received_quantity > 0 ? $item->received_quantity : $item->quantity;
    ...
    public function render()
    {
        ...
        return view('livewire.invontery.admin.purchase-order-list', compact('pendingCount', 'completedCount', 'fullyReceivedCount', 'orders'))->layout($this->layout);
    }
}
