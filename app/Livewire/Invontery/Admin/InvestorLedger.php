...
class InvestorLedger extends Component
{
    ...
    protected function calculateProfitMarginOutflow(string $month): float
    {
        ...
        // POS gross sales for the month only
        ...
        $returnsCOGS = (float) DB::table('returns_products as rp')
            ...
            ->leftJoin('product_prices as pp', 'pp.product_id', '=', 'rp.product_id')
            ->where('s.sale_type', 'pos')
            ->whereYear('s.created_at', $year)
            ->whereMonth('s.created_at', $monthNumber)
            ->selectRaw('COALESCE(SUM(rp.return_quantity * COALESCE(pp.supplier_price, 0)), 0) as total_cost')
            ->value('total_cost');
    ...
    {
        // $name is like "0.description" or "2.outflow_month"
        $parts = explode('.', (string) $name);
        if (count($parts) >= 1 && is_numeric($parts[0])) {
            $index = (int) $parts[0];
            $field = $parts[1] ?? '';

            if ($field === 'description' || $field === 'outflow_month') {
    ...
    {
        return view('livewire.invontery.admin.investor-ledger', [
            'totals' => $this->totals,
