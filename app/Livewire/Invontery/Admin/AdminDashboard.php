...
class AdminDashboard extends Component
{
    ...
    public function mount()
    {
        ...
        // Calculate sold stock value based on selling price
        $soldValue = DB::table('product_details')
            ->join('product_stocks', 'product_details.id', '=', 'product_stocks.product_id')
            ->join('product_prices', 'product_details.id', '=', 'product_prices.product_id')
            ->select(DB::raw('SUM(product_stocks.sold_count * product_prices.selling_price) as sold_value'))
            ->first();

        $this->soldValue = $soldValue->sold_value ?? 0;
    ...
    /** Separate method - called from mount() and render() so stock percentages stay fresh */
    public function loadStockStats(): void
    {
        ...
        $this->totalStock     = (float) ($stockStats->total_stock   ?? 0);
        $this->soldStock      = (float) ($stockStats->sold_stock     ?? 0);
        $this->damagedStock   = (float) ($stockStats->damaged_stock  ?? 0);
        $this->availableStock = (float) ($stockStats->available_stock ?? 0);

        // Calculate percentages based on total_stock (never inflated by returns now)
        if ($this->totalStock > 0) {
            ...
            $this->soldPercentage = $this->availablePercentage = $this->damagedPercentage = 0;
        }
    }
    ...
    {
        return view('livewire.invontery.admin.admin-dashboard')->layout($this->layout);
    }
}
